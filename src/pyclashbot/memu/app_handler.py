...
def read_apks(logger: Logger) -> list[APKFile]:
    ...
    logger.log(f"Found {len(apks)} apks in {apk_dir}")
    for apk in apks:
        logger.log(repr(apk))
    return apks
        ...
        try:
            ...
            while True:
                ...
                if any(app.startswith(apk.package_id) for app in app_list):
                    break
            logger.log(f"Installed {apk} on VM {vm_index}")
        except PyMemucException as err:
            logger.log(f"Failed to install {apk} on VM {vm_index}")
            logger.error(f"Error: {err}")
            raise err
    logger.log(f"Installed all apks on VM {vm_index}")
...
def check_for_app_install(apk_base_name: str, logger: Logger, vm_index: int):
    ...
    while True:
        try:
            ...
            logger.log(f"Waiting for VM {vm_index} to start to check for app install")
            time.sleep(1)

    found = [app for app in installed_apps if app.startswith(apk_base_name)]

    if not found:
        logger.log(f"{apk_base_name} is not installed. Please install it")
...
def check_for_clash_royale_installed(logger: Logger, vm_index: int):
    ...
    logger.log(f"Checking if Clash Royale is installed on VM {vm_index}")
    check_for_app_install(APPS["clash_royale"], logger, vm_index)
    logger.log(f"Clash Royale is installed on VM {vm_index}")
...
def check_for_drony_installed(logger: Logger, vm_index: int):
    ...
    logger.log(f"Checking if Drony is installed on VM {vm_index}")
    check_for_app_install(APPS["drony"], logger, vm_index)
    logger.log(f"Drony is installed on VM {vm_index}")
