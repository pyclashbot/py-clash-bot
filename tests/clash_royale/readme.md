...
No manual boot. Controllers boot the emulator and launch Clash during fixture
construction; if that can't be done (app missing, signed out, no main menu) the
run aborts fast with a clear message instead of hanging. The first two `SUITE`
entries then sanity-check the result:
