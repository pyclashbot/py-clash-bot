...
def find_all_cards(deck_screenshot, current_deck):
    deck_checks = dict((name_function_pair[0], name_function_pair[1])for name_function_pair in getmembers(card, isfunction) if name_function_pair[0].startswith("check_deck_for"))
    for deck_check in deck_checks:
        if deck_checks[deck_check](deck_screenshot) is not None:
            current_deck = add_card_to_deck(current_deck, deck_check.replace("check_deck_for_",""))
    return current_deck

    # UNCOMMENT THE FOLLOWING REGION IF ABOVE DOES NOT WORK
    # region oldcode
...
def check_for_card_in_hand(card):
    ...
    location = locals()[f"check_hand_for_{card}"](hand_screenshot)
    if location is not None:
        return [location[1], location[0]]
    return None

    # UNCOMMENT THE FOLLOWING REGION TO REVERT IF ABOVE DOESNT WORK
    # region oldcode
