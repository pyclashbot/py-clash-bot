...
def handle_war_attacks(logger):
    ...
    if get_to_war_page_from_main()=="restart":return "restart"
    ...
    for _ in range(5): click(220,290)
...
def click_war_icon():
    ...
    while coord is None:
        loops+=1
        if loops>8: return "failed"
        
        if random.randint(0,1)==0: scroll_up_super_fast()
        else: scroll_down_super_fast()
        time.sleep(1)
        coord=find_battle_icon_on_war_page()
