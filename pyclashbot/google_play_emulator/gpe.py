...
ADB_INSTRUCTIONS_LINK = (
    "https://docs.google.com/document/d/1tOtS39xtcIYMwFraxiEL3UhXKFPvOX_rDwON1iyxiOY/edit?usp=sharing"
)


def adb(command):
    """Runs an adb command, prints and returns its output."""
    result = subprocess.run(f"adb {command}", shell=True, capture_output=True, text=True, check=False)
...
def invalid__gpe_ratio_popup():
    layout = [
        ...
        # lets make an image
        [sg.Image(filename=r"src\pyclashbot\google_play_emulator\assets\display_ratio_tut_image.png")],
        [
            sg.Text(
...
def start_clash_royale():
    ...
    _output = adb(f"shell monkey -p {CLASH_ROYALE_PACKAGE} -c android.intent.category.LAUNCHER 1")
    print(f"Started Clash Royale in {time.time() - start_time:.2f} seconds")


def is_connected():
    """Returns True if emulator is connected and not offline."""
    result = subprocess.run("adb devices", shell=True, capture_output=True, text=True, check=False)
...
def test_abd():
    result = subprocess.run("adb devices", shell=True, capture_output=True, text=True, check=False)
    _std_out = result.stdout.strip()
    error = result.stderr.strip()
    if "'adb' is not recognized as an internal or external command" in error:
        print("Looks like adb isnt installed")
        missing_adb_installation_popup()


def connect():
    print("Connecting...")
    subprocess.run("adb disconnect localhost:6520", shell=True, check=False)
    subprocess.run("adb connect localhost:6520", shell=True, check=False)
    time.sleep(1)
...
def screenshot():
    ...
    result = subprocess.run("adb exec-out screencap -p", shell=True, capture_output=True, check=False)
...
def close_emulator():
    ...
    result = subprocess.run('taskkill /f /im "crosvm.exe"', shell=True, capture_output=True, text=True, check=False)
...
def restart_emulator(logger=None):
    ...
    if logger is None:
        ...
        logger.log("Restarting emulator...")
    ...
    while not is_emulator_running():
        if logger is None:
            ...
            logger.log("Waiting for emulator to start...")
        time.sleep(0.3)
    ...
    while find_window(GOOGLE_PLAY_PROCESS_NAME) is None:
        if logger is None:
            ...
            logger.log("Waiting for emulator window to appear...")
        time.sleep(0.3)
    ...
    if not valid_screen_size(EXPECTED_SCREEN_DIMS):
        if logger is None:
            print(
                f"[!] Warning! Emulator screen size is not {EXPECTED_SCREEN_DIMS}. Please check the emulator settings."
            )
        else:
            logger.log(
                f"[!] Warning! Emulator screen size is not {EXPECTED_SCREEN_DIMS}. Please check the emulator settings."
    ...
    if logger is None:
        ...
        logger.log("tested adb")

    if logger is None:
        print(f"Fully restarted emulator in {time.time() - start_time:.2f} seconds")
    else:
        logger.log(f"Fully restarted emulator in {time.time() - start_time:.2f} seconds")
...
def valid_gpe_ratio():
    ...
    if 0 in dims:
        print("[!] Warning! Google Play Emulator window not found or has zero dimensions.")
        return False
    ratio = dims[0] / dims[1]
