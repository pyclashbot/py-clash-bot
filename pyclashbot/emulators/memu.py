...
import numpy as np
import psutil
from pymemuc import ConfigKeys, PyMemuc, PyMemucError, VMInfo
...
class MemuEmulatorController(BaseEmulatorController):
    ...
    def _read_config_data(self):
        ...
        if debug_config_read:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] READING CONFIGURATION DATA")
            self.logger.log(f"[CONFIG] =======================================================")
        ...
        if debug_config_read:
            self.logger.log(f"[CONFIG] Loading default Memu configuration values:")
        ...
        if not file_exists:
            if debug_config_read:
                self.logger.log(f"[CONFIG] ✗ CONFIGURATION FILE NOT FOUND")
        ...
        if debug_config_read:
            self.logger.log(f"[CONFIG] ✓ CONFIGURATION FILE FOUND")
        ...
        try:
            if debug_config_read:
                self.logger.log(f"[CONFIG] Opening file for reading...")
            ...
            if debug_config_read:
                self.logger.log(f"[CONFIG] ✓ SUCCESSFULLY READ JSON FILE")
            ...
            if not file_config_data:
                if debug_config_read:
                    self.logger.log(f"[CONFIG] ⚠ EMPTY CONFIGURATION FILE")
                    self.logger.log("[CONFIG] Using all default configuration values")
                    
            if debug_config_read:
                self.logger.log(f"[CONFIG] Configuration merge summary:")
            ...
            file_read_end = time.time()
            if debug_config_read:
                self.logger.log(f"[CONFIG] ✗ JSON DECODE ERROR")
                ...
                self.logger.log(f"[CONFIG] Using all default configuration values")
        except Exception as e:
            file_read_end = time.time()
            if debug_config_read:
                self.logger.log(f"[CONFIG] ✗ FILE READ ERROR")
                self.logger.log(f"[CONFIG] File read attempt duration: {file_read_end - file_read_start:.3f}s")
        ...
        if debug_config_read:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] FINAL MERGED CONFIGURATION")
            self.logger.log(f"[CONFIG] =======================================================")
    ...
    def _set_vm_language(self):
        ...
        if debug_language:
            self.logger.log(f"[LANGUAGE] =======================================================")
            self.logger.log(f"[LANGUAGE] SETTING VM LANGUAGE TO ENGLISH")
            self.logger.log(f"[LANGUAGE] =======================================================")
        ...
        for cmd_index, command in enumerate(set_language_commands, 1):
            if debug_language:
                ...
                self.logger.log(f"[LANGUAGE] About to call: pmc.send_adb_command_vm(vm_index={self.vm_index}, command='{command}')")
            ...
            try:
                ...
                if debug_language:
                    ...
                    self.logger.log(f"[LANGUAGE] Command result: {repr(result)}")
                    self.logger.log(f"[LANGUAGE] Successful commands so far: {successful_commands}")
                    self.logger.log(f"[LANGUAGE] Sleeping for 0.33 seconds...")
        ...
        if debug_language:
            self.logger.log(f"[LANGUAGE] =======================================================")
            self.logger.log(f"[LANGUAGE] LANGUAGE SETTING SUMMARY")
            self.logger.log(f"[LANGUAGE] =======================================================")
            ...
            self.logger.log(f"[LANGUAGE] Average time per command: {(language_end - language_start)/len(set_language_commands):.3f}s")
    ...
    def _get_current_config(self) -> dict[str, str]:
        ...
        for key_index, key in enumerate(self.config.keys(), 1):
            if debug_config_read:
                ...
                self.logger.log(f"[CONFIG] -------------------------------------------------------")
                self.logger.log(f"[CONFIG] About to call: pmc.get_configuration_vm({repr(key)}, vm_index={self.vm_index})")
            ...
            try:
                ...
                if debug_config_read:
                    ...
                    self.logger.log(f"[CONFIG] Value: {repr(current_value)} (type: {type(current_value)})")
        ...
        if debug_config_read:
            ...
            self.logger.log(f"[CONFIG] Final configuration retrieved:")
    ...
    def configure(self):
        ...
        if debug_configure:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] INITIALIZING CONFIGURE METHOD")
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] Method called with parameters:")
            ...
            self.logger.log(f"[CONFIG] Debug flags active:")
            ...
            self.logger.log(f"[CONFIG] Current configuration data:")
            for key, value in self.config.items():
                self.logger.log(f"[CONFIG]   {key}: {value} (type: {type(value)})")
        
        if debug_configure:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] VALIDATING RENDER MODE PARAMETER")
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] Raw render_mode input: {repr(self.render_mode)}")
            self.logger.log(f"[CONFIG] render_mode type: {type(self.render_mode)}")
            self.logger.log(f"[CONFIG] Attempting to convert to lowercase string...")
            
        # Validate and set render mode with extreme verbosity
        render_mode_start = time.time()
        try:
            if debug_configure:
                self.logger.log(f"[CONFIG] Converting render_mode to string...")
                
            render_mode_str = str(self.render_mode)
            if debug_configure:
                self.logger.log(f"[CONFIG] str() result: {repr(render_mode_str)}")
                self.logger.log(f"[CONFIG] Applying .lower() method...")
                
            render_mode = render_mode_str.lower()
            if debug_configure:
                self.logger.log(f"[CONFIG] .lower() result: {repr(render_mode)}")
                self.logger.log(f"[CONFIG] Checking if render_mode in ['opengl', 'directx']...")
                ...
                self.logger.log(f"[CONFIG]   render_mode in ['opengl', 'directx']: {render_mode in ['opengl', 'directx']}")
            ...
            if render_mode not in ["opengl", "directx"]:
                if debug_configure:
                    ...
                    self.logger.log(f'[CONFIG] Applying fallback to "opengl"...')
                render_mode = "opengl"
            else:
                if debug_configure:
        ...
        render_mode_end = time.time()
        if debug_configure:
            self.logger.log(f'[CONFIG] ✓ RENDER MODE PROCESSING COMPLETE')
            self.logger.log(f'[CONFIG] Final render mode: {repr(render_mode)}')
        ...
        if debug_configure:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] CREATING WORKING CONFIGURATION COPY")
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] About to create copy of self.config...")
        ...
        if debug_configure:
            ...
            self.logger.log(f"[CONFIG] About to override graphics_render_mode based on render_mode...")
            self.logger.log(f"[CONFIG] Current graphics_render_mode: {working_config.get('graphics_render_mode', 'NOT_FOUND')}")
            
        if render_mode == "directx":
            old_value = working_config.get("graphics_render_mode")
            working_config["graphics_render_mode"] = 1  # directx
            if debug_configure:
                self.logger.log(f"[CONFIG] ✓ RENDER MODE OVERRIDE: DirectX")
                self.logger.log(f"[CONFIG]   Old graphics_render_mode: {old_value}")
                self.logger.log(f"[CONFIG]   New graphics_render_mode: 1 (DirectX)")
        else:
            old_value = working_config.get("graphics_render_mode")
            working_config["graphics_render_mode"] = 0  # opengl  
            if debug_configure:
                self.logger.log(f"[CONFIG] ✓ RENDER MODE OVERRIDE: OpenGL")
                self.logger.log(f"[CONFIG]   Old graphics_render_mode: {old_value}")
                self.logger.log(f"[CONFIG]   New graphics_render_mode: 0 (OpenGL)")

        # Apply each configuration setting with extreme verbosity
        if debug_configure:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] APPLYING CONFIGURATION SETTINGS TO VM")
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] Total settings to apply: {len(working_config)}")
            self.logger.log(f"[CONFIG] Target VM index: {self.vm_index}")
            self.logger.log(f"[CONFIG] About to iterate through working_config items...")
        ...
        for config_index, (key, value) in enumerate(working_config.items(), 1):
            if debug_configure:
                ...
                self.logger.log(f"[CONFIG] -------------------------------------------------------")
                self.logger.log(f"[CONFIG] Key: {repr(key)} (type: {type(key)})")
                self.logger.log(f"[CONFIG] Value: {repr(value)} (type: {type(value)})")
                self.logger.log(f"[CONFIG] Converting value to string for PyMemuc...")
                
            try:
                str_value = str(value)
                if debug_configure:
                    self.logger.log(f"[CONFIG] String value: {repr(str_value)}")
                    self.logger.log(f"[CONFIG] About to call: pmc.set_configuration_vm({repr(key)}, {repr(str_value)}, vm_index={self.vm_index})")
        ...
        if debug_configure:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] CONFIGURATION APPLICATION SUMMARY")
            self.logger.log(f"[CONFIG] =======================================================")
            ...
            self.logger.log(f"[CONFIG] Average time per setting: {(config_apply_end - config_apply_start)/len(working_config):.3f}s")

        # Set VM language with extreme verbosity
        if debug_configure or debug_language:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] SETTING VM LANGUAGE TO ENGLISH")
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] About to call self._set_vm_language()")
        ...
        try:
            ...
            if debug_configure or debug_language:
                self.logger.log(f"[CONFIG] ✓ SUCCESSFULLY SET VM LANGUAGE TO ENGLISH")
                self.logger.log(f"[CONFIG] Language setting duration: {language_end - language_start:.3f}s")
        except Exception as e:
            language_end = time.time()
            if debug_configure or debug_language:
                self.logger.log(f"[CONFIG] ✗ FAILED TO SET VM LANGUAGE")
                self.logger.log(f"[CONFIG] Exception type: {type(e)}")
                self.logger.log(f"[CONFIG] Exception message: {e}")
        ...
        if debug_configure or debug_screen:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] SETTING VM SCREEN SIZE WITH RETRY MECHANISM")
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] Target screen size: {expected_width}x{expected_height}")
            self.logger.log(f"[CONFIG] Retry attempts: 3")
        ...
        if debug_configure or debug_screen:
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] SCREEN SIZE SETTING SUMMARY")
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] Total attempts: 3")
        ...
        if debug_configure:
            ...
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] COMPLETE CONFIGURATION METHOD SUMMARY")
            self.logger.log(f"[CONFIG] =======================================================")
            ...
            self.logger.log(f"[CONFIG] Language setting: {'SUCCESS' if language_end - language_start < 30 else 'UNKNOWN'}")  
            ...
            self.logger.log(f"[CONFIG] =======================================================")
            self.logger.log(f"[CONFIG] VM CONFIGURATION COMPLETE")
            self.logger.log(f"[CONFIG] =======================================================")
    ...
    def _skip_ads(self):
        ...
        if debug_ads:
            ...
            self.logger.log(f"[ADS] Method: Send 'home' keystrokes to dismiss ads")
            self.logger.log(f"[ADS] Total attempts: 4")
            self.logger.log(f"[ADS] Sleep between attempts: 1 second")
            ...
            for i in range(4):
                ...
                try:
                    ...
                    if debug_ads:
                        ...
                        self.logger.log(f"[ADS] Keypress result: {repr(result)}")
                        self.logger.log(f"[ADS] Successful keypresses so far: {successful_keypresses}")
                        self.logger.log(f"[ADS] Sleeping for 1 second...")
    ...
    def _check_vm_size(self):
        ...
        if debug_screen:
            ...
            self.logger.log(f"[SCREEN] Method: Take screenshot and validate dimensions")
        ...
        try:
            ...
            if debug_screen:
                self.logger.log(f"[SCREEN] ✓ Home key pressed successfully")
                self.logger.log(f"[SCREEN] Keypress duration: {keypress_end - keypress_start:.3f}s")
                self.logger.log(f"[SCREEN] Waiting 2 seconds for screen to stabilize...")
            ...
            if debug_screen:
                ...
                self.logger.log(f"[SCREEN] About to call: self.screenshot()")
            ...
            if debug_screen:
                self.logger.log(f"[SCREEN] ✓ Screenshot captured successfully")
            ...
            if debug_screen:
                ...
                self.logger.log(f"[SCREEN] Extracted dimensions:")
                ...
                self.logger.log(f"[SCREEN] Expected dimensions:")
            ...
            if not dimensions_correct:
                ...
                validation_msg = f"Screen size validation FAILED: got {width}x{height}, expected {expected_width}x{expected_height}"
    ...
    def restart(self, open_clash=True, start_time=None):
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] INITIALIZING RESTART METHOD")
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] Method parameters:")
            ...
            self.logger.log(f"[RESTART] Debug flags active:")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] VALIDATING VM INDEX")
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] Current self.vm_index: {self.vm_index} (type: {type(self.vm_index)})")
            self.logger.log(f"[RESTART] Checking if vm_index is in [None, -1, '']...")
            self.logger.log(f"[RESTART]   vm_index == None: {self.vm_index == None}")
        ...
        if self.vm_index in [None, -1, '']:
            ...
            if debug_restart:
                self.logger.log(f"[RESTART] VM INDEX VALIDATION FAILED - RETURNING FALSE")
            return False
            
        if debug_restart:
            self.logger.log(f"[RESTART] ✓ VM INDEX VALIDATION PASSED")
            self.logger.log(f"[RESTART] Valid vm_index: {self.vm_index}")

        if debug_restart:
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] STARTING EMULATOR RESTART SEQUENCE")
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] Restart sequence parameters:")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] STEP 1: STOPPING ALL MEMU PROCESSES")
            self.logger.log(f"[RESTART] =====================================================")
            
        self.logger.change_status("Stopping all MEmu processes...")
        if debug_restart:
            self.logger.log(f"[RESTART] Logger status updated to: 'Stopping all MEmu processes...'")
            self.logger.log(f"[RESTART] About to call self._close_everything_memu()")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] self._close_everything_memu() completed")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] ✓ ALL MEMU PROCESSES STOPPED")
            self.logger.log(f"[RESTART] Logger status updated to: 'All MEmu processes stopped'")

        # Step 2: Configure the VM (while stopped) with extreme verbosity
        if debug_restart:
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] STEP 2: CONFIGURING VM SETTINGS (WHILE STOPPED)")
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] About to call self.configure() - this will apply all VM settings")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] Logger status updated to: 'Configuring VM settings...'")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] self.configure() completed")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] ✓ VM CONFIGURATION COMPLETE")
            self.logger.log(f"[RESTART] Logger status updated to: 'VM configuration complete'")

        # Step 3: Start the VM with extreme verbosity
        if debug_restart or debug_vm_ops:
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] STEP 3: STARTING THE VIRTUAL MACHINE")
            self.logger.log(f"[RESTART] =====================================================")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] Logger status updated to: 'Starting emulator...'")
        ...
        try:
            ...
            if debug_vm_ops:
                ...
                self.logger.log(f"[RESTART] ✓ EMULATOR START COMMAND SENT SUCCESSFULLY")
            ...
            if debug_restart:
                self.logger.log(f"[RESTART] ✗ EXCEPTION DURING VM START")
                ...
                self.logger.log(f"[RESTART] RETURNING FALSE DUE TO VM START FAILURE")
        ...
        if debug_restart or DEBUG_CONFIGURATION.get("ads", False):
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] STEP 4: SKIPPING MEMU ADS")
            self.logger.log(f"[RESTART] =====================================================")
            
        self.logger.change_status("Skipping MEmu ads...")
        if debug_restart:
            self.logger.log(f"[RESTART] Logger status updated to: 'Skipping MEmu ads...'")
            self.logger.log(f"[RESTART] About to call self._skip_ads()")
        ...
        if not ads_result:
            if debug_restart:
                self.logger.log(f"[RESTART] ✗ AD SKIP FAILED - INITIATING RECURSIVE RESTART")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] ✓ SUCCESSFULLY SKIPPED ADS")

        # Step 5: Validate screen size with extreme verbosity
        if debug_restart or DEBUG_CONFIGURATION.get("screen_size", False):
            self.logger.log(f"[RESTART] =====================================================")
            self.logger.log(f"[RESTART] STEP 5: VALIDATING SCREEN SIZE")
            self.logger.log(f"[RESTART] =====================================================")
            
        self.logger.change_status("Validating screen size...")
        if debug_restart:
            self.logger.log(f"[RESTART] Logger status updated to: 'Validating screen size...'")
            self.logger.log(f"[RESTART] About to call self._check_vm_size()")
        ...
        if not size_valid:
            if debug_restart:
                self.logger.log(f"[RESTART] ✗ SCREEN SIZE VALIDATION FAILED - INITIATING RECURSIVE RESTART")
                self.logger.log(f"[RESTART] Calling self.restart(open_clash={open_clash}, start_time={start_time})")
        ...
        if debug_restart:
            self.logger.log(f"[RESTART] ✓ SCREEN SIZE VALIDATION PASSED")

        # Step 6: Start Clash Royale if requested with extreme verbosity
        if open_clash:
            if debug_restart or debug_clash:
                self.logger.log(f"[RESTART] =====================================================")
                self.logger.log(f"[RESTART] STEP 6: STARTING CLASH ROYALE")
                self.logger.log(f"[RESTART] =====================================================")
                self.logger.log(f"[RESTART] open_clash is True, proceeding with Clash Royale startup")
                
            self.logger.change_status("Starting Clash Royale...")
            if debug_restart:
                self.logger.log(f"[RESTART] Logger status updated to: 'Starting Clash Royale...'")
                self.logger.log(f"[RESTART] About to call self.start_app('com.supercell.clashroyale')")
            ...
            try:
                ...
                if not app_start_result:
                    if debug_restart:
                        self.logger.log(f"[RESTART] ✗ CLASH ROYALE START FAILED")
                        self.logger.log(f"[RESTART] RETURNING FALSE DUE TO CLASH START FAILURE")
                    return False
                    
                if debug_restart:
                    self.logger.log(f"[RESTART] ✓ CLASH ROYALE STARTED SUCCESSFULLY")
                    
            except Exception as e:
                clash_start_end = time.time()
                if debug_restart:
                    self.logger.log(f"[RESTART] ✗ EXCEPTION DURING CLASH ROYALE START")
                    self.logger.log(f"[RESTART] Exception type: {type(e)}")
                    self.logger.log(f"[RESTART] Exception message: {e}")
            ...
            if debug_restart or debug_clash:
                self.logger.log(f"[RESTART] =====================================================")
                self.logger.log(f"[RESTART] STEP 7: WAITING FOR CLASH ROYALE MAIN MENU")
                self.logger.log(f"[RESTART] =====================================================")
                
            self.logger.change_status("Waiting for Clash Royale main menu...")
            if debug_restart:
                self.logger.log(f"[RESTART] Logger status updated to: 'Waiting for Clash Royale main menu...'")
            
            clash_main_wait_start_time = time.time()
            clash_main_wait_timeout = 240  # seconds
            
            if debug_clash:
                self.logger.log(f"[RESTART] Wait parameters:")
                ...
                self.logger.log(f"[RESTART] Initial 12-second wait before checking main menu...")
                
            time.sleep(12)  # Initial wait
            
            if debug_clash:
                self.logger.log(f"[RESTART] Initial wait complete, starting main menu detection loop...")
            ...
            while time.time() - clash_main_wait_start_time < clash_main_wait_timeout:
                ...
                if debug_clash:
                    ...
                    self.logger.log(f"[RESTART] Checking if on Clash main menu...")
                ...
                if check_if_on_clash_main_menu(self):
                    ...
                    if debug_clash:
                        self.logger.log(f"[RESTART] ✓ CLASH MAIN MENU DETECTED!")
                    ...
                    if debug_restart:
                        self.logger.log(f"[RESTART] =====================================================")
                        self.logger.log(f"[RESTART] RESTART SEQUENCE COMPLETED SUCCESSFULLY")
                        self.logger.log(f"[RESTART] =====================================================")
                ...
                menu_check_end = time.time()
                if debug_clash:
                    self.logger.log(f"[RESTART] Main menu not detected (check took {menu_check_end - menu_check_start:.3f}s)")
                    self.logger.log(f"[RESTART] Clicking deadspace at (5, 350) to handle popups...")
                ...
                if debug_clash:
                    ...
                    self.logger.log(f"[RESTART] Sleeping for 1 second before next iteration...")
            ...
            if debug_restart or debug_clash:
                self.logger.log(f"[RESTART] ✗ TIMEOUT WAITING FOR CLASH MAIN MENU")
                ...
                self.logger.log(f"[RESTART] INITIATING RECURSIVE RESTART DUE TO TIMEOUT")
        ...
        # If not opening Clash, we're done
            ...
            if debug_restart:
                self.logger.log(f"[RESTART] =====================================================")
                self.logger.log(f"[RESTART] RESTART SEQUENCE COMPLETED (NO CLASH)")
                self.logger.log(f"[RESTART] =====================================================")
                self.logger.log(f"[RESTART] open_clash was False, skipping Clash Royale startup")
                self.logger.log(f"[RESTART] Total restart time: {total_elapsed}s")
    ...
    def stop(self):
        ...
        while self._check_for_emulator_running() is True:
            if time.time() - start_time > timeout:
                self.logger.log(f"[!] Non fatal error: Timeout of {timeout} seconds reached while stopping the emulator.\n")
                return False
