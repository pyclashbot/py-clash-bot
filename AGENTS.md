...
- **`emulator.screenshot()` is BGR; card fingerprints must be extracted on the BGR path** (see `scripts/fingerprint_bgr.py`, `scripts/refresh_fingerprints_bgr.py`). Regression: `tests/test_card_fingerprint_bgr.py`.
