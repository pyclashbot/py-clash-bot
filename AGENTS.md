...
- **Output-capturing subprocess calls go through `run()` in `pyclashbot/utils/subprocess.py`** — `run(argv, timeout=...)`, an argv list with **no shell**; on timeout it kills the whole process tree (via `psutil`, identical on Windows/macOS/Linux) and returns a failed `CompletedProcess` instead of orphaning the child. Never call `subprocess.run(..., shell=True)` for a command whose output you read (e.g. ADB). Fire-and-forget launchers (emulator `Popen`, `open_folder`) are exempt — they're meant to outlive the call.
